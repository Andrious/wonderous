...
class _PhotoGalleryState extends State<PhotoGallery> {
  ...
  Offset _calculateCurrentOffset(double padding, Size size) {
    final double halfCount = (_gridSize / 2).floorToDouble();
    final Size paddedImageSize =
    ...
    final int col = _index % _gridSize;
    final int row = (_index / _gridSize).floor();
    final indexedOffset =
  ...
  void _handleSwipe(Offset dir) {
    ...
    int newIndex = _index;
    if (dir.dy != 0) {
      newIndex += _gridSize * (dir.dy > 0 ? -1 : 1);
    }
    if (dir.dx != 0) {
      newIndex += dir.dx > 0 ? -1 : 1;
    }
  ...
  Widget build(BuildContext context) {
    return ValueListenableBuilder<List<String>>(
        ...
        builder: (_, value, __) {
          ...
          final Size imgSize = (widget.imageSize ??
          ...
          return MergeSemantics(
            child: Semantics(
              ...
              child: AppBtn.basic(
                ...
                onPressed: () {
                  if (_checkCollectibleIndex(index) && selected) {
                    return;
                  }

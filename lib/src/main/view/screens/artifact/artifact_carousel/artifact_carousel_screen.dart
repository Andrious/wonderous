...
class ArtifactCarouselScreen extends StatefulWidget {
  ...
  const ArtifactCarouselScreen({Key? key, required this.type})
      : super(key: key);
...
class _ArtifactScreenState extends State<ArtifactCarouselScreen> {
  ...
  void _handleArtifactTap(int index) {
    final int delta = index - _currentOffset.round();
    if (delta == 0) {
      final HighlightData data = _artifacts[index % _artifacts.length];
  ...
  Widget _buildScreen(BuildContext context, _) {
    ...
    final double backdropWidth = w <= _maxElementWidth
        ? w
        : min(w * _partialElementWidth, _maxElementWidth);
    final double backdropHeight =
        math.min(context.heightPx * 0.65, _maxElementHeight);
    ...
    return Stack(
      children: [
        ...
        Column(
          children: [
            SimpleHeader($strings.artifactsTitleArtifacts,
                showBackBtn: false, isTransparent: true),
                    ...
                    child: PageView.builder(
                      ...
                      itemBuilder: (context, index) {
                        final bool isCurrentIndex =
                            index % _artifacts.length == _currentIndex;
                        return ExcludeSemantics(
                          excluding: isCurrentIndex == false,
                          child: MergeSemantics(
                            child: Semantics(
                              onIncrease: () =>
                                  _handleArtifactTap(_currentIndex + 1),
                              onDecrease: () =>
                                  _handleArtifactTap(_currentIndex - 1),
                              child: _CarouselItem(
                                index: index,
  ...
  Widget _buildContent(
      BuildContext context, HighlightData artifact, double width, bool small) {
    return Container(
      width: width,

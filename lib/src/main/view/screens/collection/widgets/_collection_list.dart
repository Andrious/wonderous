...
class _CollectionList extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final List<WonderData> wonders = wondersLogic.all;
    final List<Widget> children = [];
    for (int i = 0; i < wonders.length; i++) {
      final WonderData data = wonders[i];
      children.add(_buildCategoryTitle(
          context, data, data.type == scrollWonder ? scrollKey : null));
        ...
        child: ScrollDecorator.shadow(
          builder: (controller) => SingleChildScrollView(
            ...
            padding: EdgeInsets.all($styles.insets.md)
                .copyWith(bottom: $styles.insets.offset * 2.5),
            child: Column(
              children: children,
            ),
          ),
  ...
  Widget _buildCollectibleRow(
      BuildContext context, WonderType wonder, Map<String, int> states) {
    final double height = $styles.insets.lg * 6;
    final List<CollectibleData> list = collectiblesLogic.forWonder(wonder);
    if (list.isEmpty) {
      return Container(height: height, color: $styles.colors.black);
    }
    ...
    for (int i = 0; i < list.length; i++) {
      ...
      final CollectibleData collectible = list[i];
      final int state = states[collectible.id] ?? CollectibleState.lost;
      children.add(Flexible(
        child: _CollectionTile(
          ...
          onPressed: onPressed,
          heroTag:
              collectible.id == fromId ? 'collectible_image_$fromId' : null,
        ),
      ));
    }
    return SizedBox(

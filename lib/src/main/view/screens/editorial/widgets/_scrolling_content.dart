...
class _ScrollingContent extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    Text buildText(String value) =>
        Text(_fixNewlines(value), style: $styles.text.body);
    ...
    Widget buildDropCapText(String value) {
      ...
      final bool skipCaps =
          !localeLogic.isEnglish || MediaQuery.of(context).accessibleNavigation;
      return Semantics(
        label: value,
        child: !skipCaps
            ? DropCapText(
          ...
          children: [
            AppBtn.basic(
              ...
              child: Stack(children: [
                AppImage(
                    image: NetworkImage(imageUrl), fit: BoxFit.cover, scale: 1),
                Positioned.fill(
                  child: Center(
                    child: Container(
...
class _MapsThumbnail extends StatefulWidget {
  const _MapsThumbnail(this.data, {Key? key, required this.height})
      : super(key: key);
  final WonderData data;
  final double height;

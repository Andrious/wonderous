...
class _CollectionScreenState extends State<CollectionScreen>
  ...
  WonderType? get scrollTargetWonder {
    ...
    if (_states[id] != CollectibleState.discovered) {
      id = _states.keys
          .firstWhereOrNull((id) => _states[id] == CollectibleState.discovered);
    }
  ...
  void initState() {
    ...
    if (widget.fromId.isNotEmpty &&
        _states[widget.fromId] == CollectibleState.discovered) {
  ...
  void _showDetails(BuildContext context, CollectibleData collectible) {
    ...
    Future.delayed(300.ms).then((_) => collectiblesLogic.updateState(
        collectible.id, CollectibleState.explored));
  }

  Future<void> _handleReset() async {
    final String msg = $strings.collectionPopupResetConfirm;
  ...
  Widget build(BuildContext context) {
    ...
    _states.forEach((_, state) {
      ...
      if (state == CollectibleState.explored) {
        explored++;
      }
    });

    final WonderType? scrollWonder = scrollTargetWonder;
    if (scrollWonder != null) {

...
class ArchClipper extends CustomClipper<Path> {
  ...
  Path getClip(Size size) {
    ...
    final clip = Path()..moveTo(pts[0].pos.dx, pts[0].pos.dy);
...
List<ArchPoint> _getArchPts(Size size, ArchType type) {
  const double distanceFromTop = 100;
  switch (type) {
    case ArchType.pyramid:
      return [
        ArchPoint(Offset(0, size.height)),
        ArchPoint(const Offset(0, distanceFromTop)),
    ...
    case ArchType.spade:
      return [
        ...
        ArchPoint(const Offset(0, distanceFromTop)),
        ArchPoint(
            Offset(size.width / 2, 0), const Offset(0, distanceFromTop * .66)),
        ArchPoint(Offset(size.width, distanceFromTop),
    ...
    case ArchType.arch:
      return [
        ArchPoint(Offset(0, size.height)),
        ArchPoint(const Offset(0, distanceFromTop * 2)),
        ArchPoint(Offset(size.width / 2, 0), const Offset(0, 0)),
        ArchPoint(
            Offset(size.width, distanceFromTop * 2), Offset(size.width, 0)),
        ArchPoint(Offset(size.width, size.height)),
      ];

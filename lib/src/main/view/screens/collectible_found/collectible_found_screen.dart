...
class CollectibleFoundScreen extends StatelessWidget {
  ...
  Widget _buildIntro(BuildContext context) {
    final Duration t = $styles.times.fast;
  ...
  Widget _buildDetail(BuildContext context) {
    ...
    return Stack(key: const ValueKey('detail'), children: [
      Animate().custom(
          duration: t,
          builder: (context, ratio, _) => _buildGradient(context, 1, ratio)),
  ...
  Widget _buildGradient(BuildContext context, double ratioIn, double ratioOut) {
    ...
    if (ratioOut == 1) {
      return Container(color: dark.withOpacity(opacity));
    }
  ...
  Widget _buildImage(BuildContext context) {
    ...
    return AppImage(image: imageProvider, scale: 1)
        .animate()
        .custom(
          duration: t * 6,
  ...
  Widget _buildCollectionButton(BuildContext context) {
    ...
    ).animate().show(delay: t * 4).move(
        begin: Offset(0, $styles.insets.md),
        duration: t * 3,

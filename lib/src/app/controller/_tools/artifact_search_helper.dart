...
import 'package:wonders/src/model.dart' show SearchData, WonderData, WonderType;
...
class _ArtifactSearchHelperState extends State<ArtifactSearchHelper> {
  ...
  Future<void> _nextId() async {
    ...
    activeRequestCount++;
    final int id = idQueue.removeLast();
    final Uri uri = Uri.parse(_baseArtifactUri + id.toString());
    final http.Response response = await _http.get(uri);
  ...
  Future<void> _parseId(int id, Map? json) async {
    ...
    if (json == null) {
      return _logError(id, 'could not parse json');
    }
    if ((json['title'] ?? '') == '') {
      return _logError(id, 'missing title');
    }
    ...
    final String? imageUrlSmall = json['primaryImageSmall'];
    ...
    if (checkImages) {
      aspectRatio = await _getAspectRatio(imageUrlSmall);
    }
    if (aspectRatio == null) {
      return _logError(id, 'image failed to load');
    }

    final SearchData entry = SearchData(
      year,
      id,
      _escape(json['title']),
      _getKeywords(json),
  ...
  String _escape(String str) =>
      str.replaceAll("'", r"\'").replaceAll('\r', ' ').replaceAll('\n', ' ');
  ...
  Future<void> _completeIds() async {
    ...
    final String suggestions = _getSuggestions(entries);

    final Directory dir = await getApplicationDocumentsDirectory();
    final String type = wonder!.type.toString().split('.').last;
    final String path = '${dir.path}/$type.dart';
  ...
  String _getSuggestions(List<SearchData> data) {
    final HashMap<String, int> counts = HashMap<String, int>();
    final HashSet<String> ignore = HashSet<String>();
    ...
    for (int i = 0; i < data.length; i++) {
      ...
      final SearchData o = data[i];
      final RegExp re = RegExp(r'\b\w{3,}\b');
      final List<Match> matches =
          re.allMatches(o.title).toList() + re.allMatches(o.keywords).toList();
      for (int j = 0; j < matches.length; j++) {
        final String match = matches[j].group(0)!.toLowerCase();
    ...
    final int minCount = min(10, max(3, data.length / 60)).round();
  ...
  Widget _buildContent(BuildContext context) {
    return Row(
      children: [
        ...
        Expanded(
            child: ListView(
          children: log.map<Widget>(Text.new).toList(growable: false),
        )),
      ],
    );
  }
  ...
  Widget _buildWonderPicker(BuildContext context) {
    final List<String> items =
        wondersLogic.all.map<String>((o) => o.title).toList();
    items.insert(0, 'All');

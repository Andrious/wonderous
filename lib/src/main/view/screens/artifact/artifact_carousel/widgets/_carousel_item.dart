...
class _ImagePreview extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final double baseWidthScale = 1 / maxWidth;
    final double baseHeightScale = 1 / context.heightPx;

    // Size of the pages themselves.
    final double pageWidth = baseWidthScale * maxWidth * 0.65;
    final double pageHeight = baseHeightScale * maxWidth * 0.45;
    ...
    final double pageOffset = math.max(-2, math.min(2, offsetAmt));

    // Add a scale-up to the main page.
    final double midPageScaleUp =
        (1 - math.min(1, pageOffset.abs())) * mainPageScaleFactor;
    final double midPageScaleUpY =
        (1 - math.min(1, pageOffset.abs())) * mainPageScaleFactorY;
    ...
    final double yOffset =
        yOffsetFactor * ((baseHeightScale / 2) * (maxWidth / 2));

    // Apply a vertical offset based on the bottom padding provided. This includes half the element width.
    final double bottomPadding =
        (baseHeightScale / 2) * (maxHeight * 2 - (maxWidth / 2));

    final double widthFactor = pageWidth + midPageScaleUp;
    final double heightFactor = pageHeight + midPageScaleUp + midPageScaleUpY;

    final double opacity = max(0, 1 - max(0, pageOffset.abs() - 1) * 2);
        ...
        child: Opacity(
          ...
          child: Container(
            ...
            decoration: BoxDecoration(
              ...
              border: Border.all(
                  color: $styles.colors.offWhite, width: borderWidth),
